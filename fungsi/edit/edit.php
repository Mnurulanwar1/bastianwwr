...
if (!empty($_SESSION['admin'])) {
    ...
    if (!empty($_GET['stok'])) {
        ...
        // Jika stok kurang dari 1, hapus barang
        if ($stok < 1) {
            $sqlDelete = 'DELETE FROM barang WHERE id_barang=?';
            $rowDelete = $config->prepare($sqlDelete);
            $rowDelete->execute([$id]);
            echo '<script>window.location="../../index.php?page=barang&success-stok=hapus-data"</script>';
            exit(); // Keluar dari skrip setelah penghapusan barang
        }
    
        // Jika stok cukup, lakukan pembaruan stok
        $data[] = $stok;
        $data[] = $id;
